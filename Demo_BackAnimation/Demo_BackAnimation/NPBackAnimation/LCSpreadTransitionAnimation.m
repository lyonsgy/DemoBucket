...
//        _modalController.view.backgroundColor = [UIColor blueColor];
        [self initialization];
    }
    return self;
}
...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext{
    ...
    if (self.presenting) {
        ...
        UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
        toViewController.view.frame = fromViewControllerNAV.view.frame;
...
//        CGPoint point = [cell convertPoint:CGPointZero toView:fromView];
        ...
        self.contentView.frame = CGRectMake(0, self.screenBounds.size.height/2, self.screenBounds.size.width, 0);
...
//        self.cellSnapShotView.frame = CGRectMake(0, -point.y, self.cellSnapShotView.frame.size.width, self.cellSnapShotView.frame.size.height);
        self.cellSnapShotView.frame = CGRectMake(0, -self.screenBounds.size.height/2, self.cellSnapShotView.frame.size.width, self.cellSnapShotView.frame.size.height);
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    {
//        DLog(@">>>>>%.2f<<<<<<%.2f>>>>>>>",self.gesture.scrollview.contentOffset.y+ScreenWidth/375*211,self.gesture.scrollview.contentSize.height + 64 +216 + self.topMargin);
        if (self.gesture.scrollview.contentOffset.y+ScreenWidth/375*211+100 < -self.topMargin) {
            self.gesture.direction = LCSpreadTransitionDirectionBottom;
            [self.modalController dismissViewControllerAnimated:YES completion:NULL];
        }else if (self.gesture.scrollview.contentOffset.y + self.gesture.scrollview.bounds.size.height >= self.gesture.scrollview.contentSize.height + 100 +216+ self.topMargin){
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (isVerticalGesture && (nowPoint.y > prevPoint.y) && (ScreenWidth/375*211+self.scrollview.contentOffset.y+100 <= topVerticalOffset)) {
        self.direction = LCSpreadTransitionDirectionBottom;
        self.isFail = @YES;
    }
    else if (isVerticalGesture && (nowPoint.y <= prevPoint.y) && (self.scrollview.contentOffset.y + self.scrollview.bounds.size.height >= self.scrollview.contentSize.height+216+100)) {
